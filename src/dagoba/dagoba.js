...
Dagoba.G.searchVertices = function(filter) {                      // find vertices that match obj's key-value pairs
  return this.vertices.filter((vertex) => Dagoba.objectFilter(vertex, filter))
}
...
Dagoba.fromString = function(str) {                               // another graph constructor
  const obj = Dagoba.parseJSON(str)                                 // this could throw
  if(!obj) return null
...
Dagoba.Q.run = function() {                                       // our virtual machine for query processing
  ...
  const max = this.program.length - 1                               // last step in the program
  let maybe_gremlin = false                                       // a gremlin, a signal string, or false
  let results = []                                                // results for this particular run
  let done = -1                                                   // behindwhich things have finished
  let pc = max                                                    // our program counter -- we start from the end

  let step; let state; let pipetype
  ...
  results = results.map((gremlin) =>                        // return either results (like property('name')) or vertices
     gremlin.result != null
...
Dagoba.getPipetype = function(name) {
  const pipetype = Dagoba.Pipetypes[name]                           // a pipe type is just a function
...
Dagoba.addPipetype('vertex', (graph, args, gremlin, state) => {
  if(!state.vertices)
...
Dagoba.addPipetype('outAllN', (graph, args, gremlin, state) => {
  ...
  const vertex = state.edgeList[state.current].pop()._in
...
Dagoba.addPipetype('inAllN', (graph, args, gremlin, state) => {
  ...
  const vertex = state.edgeList[state.current].pop()._out
...
Dagoba.addPipetype('property', (graph, args, gremlin, state) => {
  if(!gremlin) return 'pull'                                      // query initialization
  gremlin.result = gremlin.vertex[args[0]]
  return gremlin.result == null ? false : gremlin                 // undefined or null properties kill the gremlin
})

Dagoba.addPipetype('unique', (graph, args, gremlin, state) => {
  if(!gremlin) return 'pull'                                      // query initialization
...
Dagoba.addPipetype('filter', (graph, args, gremlin, state) => {
  if(!gremlin) return 'pull'                                      // query initialization
...
Dagoba.addPipetype('take', (graph, args, gremlin, state) => {
...
Dagoba.addPipetype('as', (graph, args, gremlin, state) => {
  if(!gremlin) return 'pull'                                      // query initialization
...
Dagoba.makeGremlin = function(vertex, state) {                    // gremlins are simple creatures:
  return {vertex, state: state || {} }                    // a current vertex, and some state
}
...
Dagoba.clone = function(graph) {
  const G = Dagoba.cloneflat(graph)
...
Dagoba.addTransformer = function(fun, priority) {
  ...
  Dagoba.T.splice(i, 0, {priority, fun})
}

Dagoba.transform = function(program) {
  return Dagoba.T.reduce((acc, transformer) => transformer.fun(acc), program)
}


Dagoba.addAlias = function(newname, newprogram) {
  Dagoba.addPipetype(newname, () => {})                      // because there's no method catchall in js
  newprogram = newprogram.map((step) =>
     [step[0], step.slice(1)]                               // [['out', 'parent']] => [['out', ['parent']]]
  )
  // defaults = defaults || []                                    // default arguments for the alias
  Dagoba.addTransformer((program) =>
     program.reduce((acc, step) => {
