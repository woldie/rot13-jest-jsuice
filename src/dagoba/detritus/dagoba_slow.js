...
DagobaSlow.query = function(graph) { 
  const query = Object.create(DagobaSlow.Query) 
...
DagobaSlow.Query.run = function() {
  ...
  while(gremlins.length) {
    const gremlinbox = gremlins.pop()
    const result = stepper(gremlinbox[0], gremlinbox[1])
  ...
  const collection = this.state[this.state.length - 1] || []
  this.result = collection.map((gremlin) => gremlin.vertex)
...
const methods = ['out', 'in', 'attr', 'outV', 'outE', 'inV', 'inE', 'both', 'bothV', 'bothE', 'filter']
methods.forEach((name) => {DagobaSlow.Query[name] = DagobaSlow.make_fun(name)})
...
DagobaSlow.Funs = {
  vertex(graph, args, gremlin, state) {
    ...
    const vert = gremlin.state.pop()
    const vertex = graph.findVertexById(vert._id)
    const clone = DagobaSlow.make_gremlin(vertex)
  ...
  out(graph, args, gremlin, state) {
    ...
    if(gremlin.state.length == 0)
      return {} // original gremlin dies here...
    const vertex = graph.findVertexById(gremlin.state.pop()._in)
    const clone = DagobaSlow.make_gremlin(vertex)
    return {stay: [gremlin], go: [clone]}
  },
  ...
  'in': function(graph, args, gremlin, state) {
    ...
    if(gremlin.state.length == 0)
      return {} // original gremlin dies here...
    const vertex = graph.findVertexById(gremlin.state.pop()._out)
    const clone = DagobaSlow.make_gremlin(vertex)
    return {stay: [gremlin], go: [clone]}
  },
  ...
  collector(graph, args, gremlin, state) {
...
DagobaSlow.firehooks = function(type, query) {
  ...
  return ((DagobaSlow.hooks || {})[type] || []).reduce((acc, callback) => callback.apply(query, acc), args)
}
...
DagobaSlow.addhook('postquery', DagobaSlow.uniqueify = function (results) { // THINK: should we inline this and merge&count in the gremlins?
  return [results.filter((item, index, array) => array.indexOf(item) == index)]
})

DagobaSlow.addhook('postquery', DagobaSlow.cleanclone = function (results) { // THINK: do we always want this?
  return [results.map((item) => JSON.parse(JSON.stringify(item, (key, value) => key[0]=='_' ? undefined : value)))]
})
...
DagobaSlow.Graph.v = function() {
  const query = DagobaSlow.query(this)
  query.add(['vertex'].concat( [].slice.call(arguments) ))
  return query
}

DagobaSlow.graph = function() { 
  const graph = Object.create( DagobaSlow.Graph ) 
...
DagobaSlow.Graph.findVertexById = function(vertex_id) {
  return this.vertices.first((vertex) => vertex._id == vertex_id)
}
...
DagobaSlow.Graph.searchVertices = function(obj) {
  return this.vertices.filter(
    (vertex) => Object.keys(obj[0]).reduce(
        (acc, key) => acc && obj[0][key] == vertex[key], true ) ) }

DagobaSlow.Graph.findEdgeById = function(edge_id) {
  return this.edges.first((edge) => edge._id == edge_id) }

DagobaSlow.Graph.findOutEdges = function(vertex) {
  return this.edges.filter((edge) => edge._out == vertex._id) }

DagobaSlow.Graph.findInEdges = function(vertex) {
  return this.edges.filter((edge) => edge._in == vertex._id) }
